...
export function getConfig(workspaceConfigs: object[] = []): Config {
  ...
  const workspaceMappings: Mapping[] = workspaceConfigs.reduce<Mapping[]>(
    (previous, config) => {
...
export function getWorkspaceConfigs(): PromiseLike<object[]> {
  return workspace
    .findFiles('[tj]sconfig.json', '**/node_modules/**')
    .then((files) => {
      if (files && files.length > 0) {
        return files.map((file) =>
          JSON5.parse(readFileSync(file.fsPath).toString())
        )
      } else {
        return []
      }
...
function getMappings(configuration: WorkspaceConfiguration): Mapping[] {
  ...
  return Object.keys(mappings)
    ...
    .filter(
      (mapping) =>
        !!workspace.rootPath || mapping.value.indexOf('${workspaceRoot}') === -1
    )
    .map((mapping) => ({
      key: mapping.key,
      value: mapping.value.replace('${workspaceRoot}', workspace.rootPath)
    }))
}
