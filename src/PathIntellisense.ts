...
export class PathIntellisense implements CompletionItemProvider {
  ...
  provideCompletionItems(
    ...
    const request: Request = {
      ...
      textCurrentLine,
      textWithinString: getTextWithinString(
        textCurrentLine,
        position.character
      ),
    ...
    return this.shouldProvide(request)
  ...
  shouldProvide(request: Request) {
    const typedAnything =
      request.textWithinString && request.textWithinString.length > 0
    const startsWithDot = typedAnything && request.textWithinString[0] === '.'
    const startsWithMapping =
      typedAnything &&
      request.config.mappings.some(
        (mapping) => request.textWithinString.indexOf(mapping.key) === 0
      )
  ...
  provide(request: Request) {
    const path = getPath(
      request.fileName,
      request.textWithinString,
      request.config.absolutePathToWorkspace ? workspace.rootPath : null,
      request.config.mappings
    )
