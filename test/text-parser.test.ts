...
suite('Text Parser Tests', () => {
  ...
  test('importStringRange', () => {
    const requireRange = importStringRange(
      "const test = require('./file')",
      new Position(0, 28)
    )
    assert.equal(
      requireRange.end.line,
      requireRange.start.line,
      'Require range line index'
    )
    assert.equal(
      requireRange.start.character,
      24,
      'Require range start character'
    )
    assert.equal(requireRange.end.character, 28, 'Require range end character')

    const importRange = importStringRange(
      "import testing from './some-file';",
      new Position(0, 33)
    )
    assert.equal(
      importRange.end.line,
      importRange.start.line,
      'Import range line index'
    )
    assert.equal(
      importRange.start.character,
      23,
      'Import range start character'
    )
